...
namespace Egzamin
{
    ...
    class Paczkomatowa : Przesylka
    {

        public Paczkomatowa(Nadawca n, Odbiorca o, int waga, int dlugosc, int szerokosc, int wysokosc, DateTime data, Paczkomat pacz)
        {
            nadawca = n;
            odbiorca = o;
            Waga = waga;
            if (Dlugosc > Poczta.CenaWymiar[Poczta.CenaWymiar.Count - 1].x || Szerokosc > Poczta.CenaWymiar[Poczta.CenaWymiar.Count - 1].y || Wysokosc > Poczta.CenaWymiar[Poczta.CenaWymiar.Count - 1].z)
                throw new Exception("Za duży wymiar paczki");
            Dlugosc = dlugosc;
            Szerokosc = szerokosc;
            Wysokosc = wysokosc;
            DataNadania = data;
        ...
        protected Paczkomat paczkomat { get; set; }
...

        {
            ...
            int waga3 = Poczta.CenaWagi[1].waga;
            ...
            if (objPaczki <= obj1) kosztPrzesylkiB = Poczta.CenaWymiar[0].cena;
            else if (objPaczki > obj1 && objPaczki <= obj2) kosztPrzesylkiB = Poczta.CenaWymiar[1].cena;
            else if (objPaczki >= obj3) kosztPrzesylkiB = Poczta.CenaWymiar[2].cena;

            if (kosztPrzesylkiA < kosztPrzesylkiB) return kosztPrzesylkiA;
            else return kosztPrzesylkiB;
